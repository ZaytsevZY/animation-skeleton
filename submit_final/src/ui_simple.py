...
骨架绑定 UI - 完整版（含导出功能）
功能：
- 左侧工具栏：重置按钮、蒙皮模式切换、导出功能
- 性能优化：Actor缓存、延迟更新
- 导出功能：骨架关节点位置、连接关系、绑定权重
"""
...
class AnimationExporter(QThread):
    ...
    def create_walking_animation(self, num_frames=120):
        """创建摇头+奔跑动画"""
        self.status_message.emit("🎬 创建摇头+奔跑动画...")

        # 分析骨架中每个关节的角色
        joint_roles = [self.get_joint_role(self.skeleton, i) for i in range(self.skeleton.n)]

        animations = []

        # 定义两个阶段：前60帧奔跑，后60帧摇头
        run_frames = 60
        shake_frames = 60

        for frame in range(num_frames):
            self.progress_updated.emit(int((frame + 1) / num_frames * 100))

            # 判断当前阶段
            if frame < run_frames:
                # 奔跑阶段
                t = frame / run_frames * 4 * np.pi  # 奔跑用4π，完成2个完整周期
                is_running = True
            else:
                # 摇头阶段
                t = (frame - run_frames) / shake_frames * 2 * np.pi  # 摇头用2π，完成1个完整周期
                is_running = False
            ...
            for i in range(self.skeleton.n):
                ...
                if role == 'root':
                    if is_running:
                        # 奔跑阶段沿Y负方向前进
                        forward = np.array([0, -frame / run_frames * 0.5,  # 总前进距离0.5单位
                                          np.sin(t * 2) * 0.02])  # 上下起伏
                        T = self.create_transform_matrix(t=forward)
                ...
                # 躯干
                    ...
                    if is_running:
                        ...
                        T = self.create_transform_matrix(R=R)

                # 后腿髋部
                elif role in ['l_hip0', 'r_hip0']:
                    if is_running:
                        phase = 0 if 'l_' in role else np.pi
                        angle = np.sin(t + phase) * 0.35
    ...
    def run(self):
        ...
        try:
            ...
            self.status_message.emit("🎨 渲染动画帧...")
            frame_files = []

            for frame_idx, local_transforms in enumerate(animations):
                self.status_message.emit(f"   渲染第 {frame_idx+1}/{num_frames} 帧...")
            ...
            # 生成GIF
            self.status_message.emit("📹 生成GIF动画...")
            ...
            self.status_message.emit("✅ GIF动画已保存")

            # 尝试生成MP4
            self.status_message.emit("📹 生成MP4视频...")
            mp4_path = os.path.join(script_dir, "output", "cow_animation.mp4")
            try:
                cmd = (f"ffmpeg -y -framerate 10 -i {output_dir}/frame_%04d.png "
                       f"-vf 'scale=1000:800' -c:v libx264 -pix_fmt yuv420p {mp4_path}")
                result = os.system(cmd)

                if result == 0 and os.path.exists(mp4_path):
                    self.status_message.emit("✅ MP4视频已保存")
                else:
                    cmd_backup = (f"ffmpeg -y -framerate 10 -i {output_dir}/frame_%04d.png "
                    ...
                    if result == 0 and os.path.exists(mp4_path):
                        ...
                        self.status_message.emit("⚠️ MP4生成失败")
            except Exception as e:
                self.status_message.emit(f"⚠️ MP4生成出错: {str(e)}")

            self.finished_signal.emit(True, f"动画导出完成！\nGIF: {gif_path}\nMP4: {mp4_path}")

        except Exception as e:
            error_msg = f"动画导出失败：{str(e)}"
            self.status_message.emit(error_msg)
            self.finished_signal.emit(False, error_msg)


class CompleteDragUI(QMainWindow):
    """完整版骨架绑定UI - 含导出功能"""
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle("骨架绑定工具 - 完整版")
    ...
    def create_toolbar(self):
        ...
        intro_text.setStyleSheet(
            "font-size: 11px; color: #444; line-height: 1.4; padding: 8px;"
        )
        ...
        self.export_animation_button.setObjectName("exportButton")
        ...
        self.export_skeleton_button.setObjectName("exportButton")
        ...
        self.export_weights_button.setObjectName("exportButton")
        self.export_weights_button.clicked.connect(self.export_weights_info)
        self.export_weights_button.setToolTip("导出绑定权重信息")
        export_layout.addWidget(self.export_weights_button)

        # 导出所有信息按钮
        self.export_all_button = QPushButton("📁 导出全部信息")
        self.export_all_button.setObjectName("exportButton")
        ...
        try:
            ...
            for i, joint in enumerate(self.skeleton.joints):
                joint_info = {
                    ...
                    "name": joint.name,
                    "parent": joint.parent,
                    "bind_position": joint.pos.tolist(),
                    "current_position": current_joint_positions[i].tolist(),
                    "transform": self.joint_transforms[i].tolist()
                }
                skeleton_info["joints"].append(joint_info)
            ...
            for jp, jc in self.bones:
                ...
                skeleton_info["bones"].append(bone_info)

            # 保存文件
            filename = f"skeleton_info_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            filepath = os.path.join(self.output_dir, filename)

            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(skeleton_info, f, indent=2, ensure_ascii=False)

            self.statusBar().showMessage(f"✅ 骨架信息已导出：{filename}")
            print(f"📋 骨架信息已导出到：{filepath}")

            QMessageBox.information(self, "成功", f"骨架信息已导出到：\n{filepath}")

        except Exception as e:
            error_msg = f"导出骨架信息失败：{str(e)}"
            print(error_msg)
    ...
    def export_weights_info(self):
        ...
        try:
            ...
            weights_info = {
                ...
                "metadata": {
                    "timestamp": datetime.now().isoformat(),
                    "num_vertices": len(self.mesh.v),
                    "num_joints": self.skeleton.n
                }
            }

            # 顶点信息
            for i, vertex in enumerate(self.mesh.v):
                vertex_info = {
                    "index": i,
                    "position": vertex.tolist()
                }
                weights_info["vertices"].append(vertex_info)

            # 完整权重
            for i, vertex_weights in enumerate(self.weights):
                weights_data = {
                    "vertex_index": i,
                    "weights": {}
                }
                ...
                for j, weight in enumerate(vertex_weights):
                    if weight > 0.001:  # 忽略非常小的权重
                        weights_data["weights"][j] = float(weight)
                weights_info["weights"]["full"].append(weights_data)

            # 简化权重
            for i, vertex_weights in enumerate(self.simple_weights):
                weights_data = {
                    "vertex_index": i,
                    "weights": {}
                }
                for j, weight in enumerate(vertex_weights):
                    if weight > 0:
                        weights_data["weights"][j] = float(weight)
                weights_info["weights"]["simple"].append(weights_data)

            # 保存文件
            filename = f"weights_info_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            filepath = os.path.join(self.output_dir, filename)

            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(weights_info, f, indent=2, ensure_ascii=False)

            self.statusBar().showMessage(f"✅ 权重信息已导出：{filename}")
            print(f"⚖️ 权重信息已导出到：{filepath}")

            QMessageBox.information(self, "成功", f"权重信息已导出到：\n{filepath}")

        except Exception as e:
            error_msg = f"导出权重信息失败：{str(e)}"
            print(error_msg)
            QMessageBox.critical(self, "错误", error_msg)
        ...
        try:
            ...
            all_info = {
                "project_info": {
                    ...
                    "description": "包含骨架、权重、顶点等完整信息"
                },
                "mesh": {
                    "num_vertices": len(self.mesh.v),
                    "num_faces": len(self.mesh.f),
                    "vertices": self.mesh.v.tolist(),
                    "faces": self.mesh.f.tolist()
                },
                "skeleton": {
                    "num_joints": self.skeleton.n,
                    "num_bones": len(self.bones),
                ...
                "weights": {
                    "full": self.weights.tolist(),
                    "simple": self.simple_weights.tolist()
                },
                "transforms": {
                    "initial_transforms": self.initial_joint_transforms.tolist(),
                    "current_transforms": self.joint_transforms.tolist()
                }
            }
            ...
            for i, joint in enumerate(self.skeleton.joints):
                ...
                all_info["skeleton"]["joints"].append(joint_info)
            ...
            for jp, jc in self.bones:
                ...
                all_info["skeleton"]["bones"].append(bone_info)

            # 保存文件
            filename = f"complete_info_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            filepath = os.path.join(self.output_dir, filename)

            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(all_info, f, indent=2, ensure_ascii=False)

            self.statusBar().showMessage(f"✅ 所有信息已导出：{filename}")
            print(f"📁 所有信息已导出到：{filepath}")

            QMessageBox.information(self, "成功", f"所有信息已导出到：\n{filepath}")

        except Exception as e:
            error_msg = f"导出所有信息失败：{str(e)}"
            print(error_msg)
            QMessageBox.critical(self, "错误", error_msg)
    ...
    def on_animation_export_finished(self, success, message):
        ...
        if success:
            ...
            self.statusBar().showMessage("✅ 动画导出完成")
        else:
            QMessageBox.critical(self, "错误", message)
            self.statusBar().showMessage("❌ 动画导出失败")
    ...
    def update_gizmo_only(self):
        ...
        joint_name = self.skeleton.joints[self.selected_joint].name
        sphere_radius = np.linalg.norm(self.mesh.v.max(axis=0) - self.mesh.v.min(axis=0)) * 0.015
